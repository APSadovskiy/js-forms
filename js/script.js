...
/*console.log(mainFormInput.value.split('.')[1].length); */
/* 
if(mainFormInput.value.split('.')[1].length>2) */
/* document.getElementById('input_fot').addEventListener('keydown',
(ev)=> {
	if(ev.key==',') ev.key.value='.'; //Не работает 
	if(ev.key.length==1 && (ev.key<'0' || ev.key>'9' ))  if(ev.key != '.')  ev.preventDefault();
...
(ev)=> {
 ...
 if(ev.key == '.' || ev.key == ',') ev.target.value= parseFloat(mainFormInput_fot.value).toFixed(2);

}) 

document.getElementById('input_fot').onchange = e =>{e.target.value= parseFloat(mainFormInput_fot.value).toFixed(2);}

 document.getElementById('input_fot').oninput = e =>{

	//e.target.value= parseFloat(mainFormInput_fot.value).toFixed(2);
	if(mainFormInput_fot.value.split('.')[1].length>2) {e.target.value=mainFormInput_fot.value.slice(0,-1);	}

} 
document.getElementById('input_fot').onkeyup = e =>{	
	//mainFormInput_fss.value=(mainFormInput_fot.value)/20;

const yearFOT= mainFormInput_fot.value*12;
...
mainFormInput_fss.value=itogFSSPFR;
